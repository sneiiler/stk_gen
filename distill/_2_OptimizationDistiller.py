...
# 获取API配置
api_base_openai = os.getenv("GEMINI_API_BASE")
api_key_openai = os.getenv("GEMINI_API_KEY")
# api_base_openai = "http://127.0.0.1:11400/v1/"
# api_key_openai = "sk-123456"
print(f"API配置: {'✓' if api_key_openai and api_base_openai else '✗'}")
print(f"API配置: {api_key_openai} and {api_base_openai}")

if not api_key_openai or not api_base_openai:
    raise ValueError("未找到API配置，请检查环境变量 GEMINI_API_KEY 和 GEMINI_API_BASE")
...
class OptimizationDistiller:
    ...
    def _extract_reasoning_and_content(self, response_stream) -> tuple[str, str]:
        ...
        if "gemini" in self.model_name.lower():
            ...
            if thought_match:
                ...
                # 如果没有通过reasoning_content获取到内容，或者thought内容更丰富，则使用thought内容
                if not reasoning_content.strip() or len(thought_content) > len(
                    reasoning_content
                ):
                    reasoning_content = thought_content
                else:
                    # 如果都有内容且reasoning_content更长，可以选择合并或保留reasoning_content
                    # 这里选择保留原有的reasoning_content，避免重复
                    pass
    ...
    def optimize_single_validation_item(
        ...
        try:
            ...
            # 调用API，针对Gemini模型启用thinking_config
            api_params = {
