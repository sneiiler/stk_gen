...
class RawConstellationDataModel(BaseModel):
    ...
    history_cluster_result: Optional[List[List[ClusterInfo]]] = Field(..., description="上n次分簇结果")


class SatelliteClusterOutput(BaseModel):
    """卫星分簇划分输出模型"""
    chain_of_thought: Optional[str] = Field(description="推理过程，大模型生成阶段不需要填写，后期封装")
    clusters: List[ClusterInfo] = Field(description="划分的卫星分簇列表")
    
    def to_think_json(self):
        thought_content = self.chain_of_thought or ""
        return "<think>" + thought_content + "</think>" \
...
class LLMConversationMessage(BaseModel):
    ...
    response: SatelliteClusterOutput = Field(..., description="包含推理过程和分簇结果的完整响应")
    ...
    def to_sharegpt_json(self) -> str:
        ...
        sharegpt_data = {
            "messages": [
                ...
                {
                    "role": "user",
                    "content": self.input.model_dump_json()
                },
                {
                    "role": "assistant",
                    "content": self.response.to_think_json()
                }
            ]
        }
