...
for route_idx, (base, target, distance) in enumerate(routes):
    ...
    for _ in range(num_missiles):
        ...
        altitude = min(round(base_altitude * random.uniform(0.9, 1.1), 2), 1500.0)  # Ensure altitude does not exceed 2500 km   
        entry = {
...
output_dir = get_data_dir()/ f"missile_route_info_v{current_timestamp}.json"
